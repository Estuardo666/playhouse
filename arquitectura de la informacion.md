# PLAYHOUSE — Arquitectura de información y contenido base para one page

> Documento listo para pasar a VSCode como base de maquetación y construcción de la landing page de **Playhouse – Teatro Educativo**.
> 
> En este archivo ya no se usan frases vagas como “contenido sugerido” o “bloques sugeridos”. Cada sección contiene **contenido concreto**, títulos, pretítulos, textos base, grupos de información, CTAs y estructura real a partir del material proporcionado.

---

## 0. Navegación principal

```txt
Inicio
Sobre Playhouse
Misión y Visión
Espectáculos
Talleres
Residencia Artística
Materiales
Equipo
Galería
Contacto
```

---

## 1. Hero principal

**Section ID:** `hero`  
**Pretítulo:** PLAYHOUSE · Teatro Educativo  
**Título:** El inglés cobra vida en el escenario  

**Texto base:**
Playhouse es un proyecto pionero en teatro educativo en inglés que combina arte, música y pedagogía para crear experiencias de aprendizaje inmersivas, creativas y memorables para niños, jóvenes, docentes, instituciones y familias.

**Texto de apoyo:**
Más de 15 años de experiencia en artes escénicas y enseñanza respaldan una propuesta que transforma el escenario en un aula viva y convierte el aprendizaje del inglés en una experiencia real, emocionante y participativa.

**Highlights del hero:**
- Teatro + música + pedagogía
- Aprendizaje inmersivo en inglés
- Experiencias para escuelas, estudiantes y familias

**CTA principal:**
- Texto: `Contáctanos`
- Link: `https://forms.gle/ePJY7c7sok2NaBWBA`

**CTA secundario:**
- Texto: `Ver espectáculos`
- Link: `#espectaculos`

**Redes visibles en hero o header expandido:**
- Instagram: `@playhouse.ec`
- Facebook: `playhousec`
- TikTok: `playhouse.ec`
- YouTube: `@playhouse_ec`

---

## 2. Sobre Playhouse

**Section ID:** `about`  
**Pretítulo:** Sobre nosotros  
**Título:** Unimos artes escénicas y aprendizaje del inglés en un mismo escenario  

**Párrafo 1:**
Playhouse fue fundada con la misión de integrar el teatro y el aprendizaje del inglés en una sola experiencia. Somos una productora teatral comprometida con crear propuestas que inspiran, educan y entretienen a públicos de todas las edades.

**Párrafo 2:**
Creemos que el teatro es un puente poderoso para aprender. Cada obra es una oportunidad para sumergirse en un idioma, una cultura y una historia. A través de talleres, musicales y actuaciones en inglés, ofrecemos un espacio donde el idioma cobra vida de forma natural, divertida y emocionante.

**Párrafo 3:**
Nuestro trabajo combina pasión por el teatro con un enfoque pedagógico innovador. Reunimos actores, directores, músicos, coreógrafos y creativos que comparten una misma visión: convertir el escenario en un aula viva y al público en un participante activo.

**Párrafo 4:**
Más que producir espectáculos, creamos experiencias que dejan huella, fomentan la creatividad y fortalecen la confianza para comunicarse en inglés mientras celebran la magia del teatro.

**Pilares de marca:**
- Aprendizaje inmersivo
- Expresión artística
- Confianza comunicativa
- Desarrollo lingüístico, cultural y social

**Dato destacado:**
- Más de 15 años de experiencia en artes escénicas y enseñanza

---

## 3. Misión y Visión

**Section ID:** `purpose`  
**Pretítulo:** Nuestra esencia  
**Título:** Una propuesta artística y pedagógica con visión de crecimiento nacional  

### Misión
Promover el aprendizaje del inglés y el desarrollo artístico a través del teatro musical, creando experiencias escénicas inmersivas y divertidas que inspiran a niños, jóvenes y adultos a expresarse, comunicarse y crecer en un entorno creativo y bilingüe.

### Visión
Ser un referente nacional en teatro educativo en inglés, con producciones anuales que lleguen a diversas ciudades, destacándose por combinar arte y pedagogía en propuestas escénicas innovadoras y creciendo mediante alianzas estratégicas y materiales educativos propios.

**Valores que se desprenden del proyecto:**
- Creatividad
- Inmersión
- Colaboración
- Expresión
- Innovación pedagógica
- Confianza escénica

---

## 4. Espectáculos

**Section ID:** `espectaculos`  
**Pretítulo:** Nuestros espectáculos  
**Título:** Obras en inglés que entretienen, enseñan y conectan con el público  

**Texto introductorio:**
Nuestros espectáculos están pensados para acercar el inglés a estudiantes y audiencias de forma emocional, dinámica y memorable. Cada obra combina narrativa, música, actuación y propósito pedagógico.

---

### 4.1. Show destacado: The Three Little Pigs

**Pretítulo del bloque:** Musical familiar  
**Título del bloque:** The Three Little Pigs – A Musical Tale  

**Sinopsis:**
Esta reinvención entrañable y humorística del cuento clásico presenta a tres hermanos cerditos que deben enfrentar los retos de crecer, construir sus propios hogares y burlar a un lobo hambriento. Con canciones originales y un enfoque renovado, la historia conecta con niños y adultos por igual.

**Valor educativo:**
La obra promueve trabajo en equipo, ingenio, comprensión narrativa, conexión con el idioma inglés y participación emocional del público.

**Preguntas dramáticas que pueden aparecer como apoyo visual:**
- ¿Derribará el lobo la casa de los cerditos?
- ¿Lograrán escapar una vez más?
- ¿Puede un Gran Lobo Feroz cambiar de verdad?

**CTA del bloque:**
- Texto: `Solicitar esta función`

---

### 4.2. Show destacado: The Trash Talk

**Pretítulo del bloque:** Teatro breve con conciencia ambiental  
**Título del bloque:** The Trash Talk  

**Sinopsis:**
Dos estudiantes muy distintos reciben servicio comunitario y son enviados a limpiar un parque contaminado junto al río. Jake, obsesionado con el medio ambiente, se toma la tarea en serio. Leo, el bromista de la clase, preferiría hacer chistes antes que separar botellas y latas. Bajo la mirada del Sr. Torres, ambos pasan del conflicto al trabajo en equipo.

**Valor educativo:**
La obra muestra cómo acciones pequeñas pueden generar cambios reales, promueve conciencia ambiental y convierte el aprendizaje del inglés en una experiencia teatral breve, divertida y significativa.

**Dato clave:**
- Formato aproximado: 15 minutos

**CTA del bloque:**
- Texto: `Quiero más información`

---

## 5. Talleres

**Section ID:** `talleres`  
**Pretítulo:** Talleres en inglés  
**Título:** Experiencias formativas que desarrollan voz, cuerpo, creatividad y confianza  

**Texto introductorio:**
En Playhouse, los talleres usan el poder del teatro para hacer del aprendizaje del inglés una experiencia divertida, creativa y memorable. A través de juegos, improvisación, narración, música y actuación, los estudiantes descubren nuevas formas de expresarse mientras fortalecen sus habilidades comunicativas.

---

### 5.1. Introducción al Teatro

**Pretítulo del bloque:** Fundamentos escénicos  
**Título del bloque:** Introducción al Teatro  

**Descripción:**
Un taller ideal para que los estudiantes se acerquen al inglés de manera lúdica usando cuerpo, voz y emociones como herramientas expresivas.

**Objetivos:**
- Aprender fundamentos del teatro: voz, cuerpo, espacio y personaje
- Fomentar trabajo colaborativo en inglés
- Desarrollar confianza y espontaneidad oral

**Resultados esperados:**
- Participación activa en dinámicas teatrales en inglés
- Desarrollo inicial de personajes y escenas simples
- Mayor seguridad en la expresión oral

---

### 5.2. Storytelling

**Pretítulo del bloque:** Narración creativa  
**Título del bloque:** Storytelling  

**Descripción:**
La narración fortalece el inglés desde la emoción y la estructura de las historias. Este taller ayuda a construir personajes, tramas y relatos originales en inglés.

**Objetivos:**
- Identificar inicio, conflicto, clímax y resolución
- Crear personajes y tramas originales
- Desarrollar habilidades de narración oral y performativa

**Resultados esperados:**
- Creación de historias breves en inglés
- Mejor organización y expresión de ideas
- Desarrollo de imaginación y creatividad

---

### 5.3. Teatro de Improvisación

**Pretítulo del bloque:** Fluidez y espontaneidad  
**Título del bloque:** Teatro de Improvisación  

**Descripción:**
La improvisación fortalece reacción, escucha activa y creatividad. En inglés, ayuda a reducir miedo al error y mejora la fluidez hablada.

**Objetivos:**
- Fortalecer capacidad de reacción en inglés
- Estimular creatividad y uso espontáneo del idioma
- Construir confianza en la interacción oral

**Resultados esperados:**
- Mayor fluidez en inglés hablado
- Desarrollo de imaginación y colaboración grupal
- Actitud positiva frente al error y al aprendizaje

---

### 5.4. Introducción al Teatro Musical

**Pretítulo del bloque:** Voz, movimiento y actuación  
**Título del bloque:** Introducción al Teatro Musical  

**Descripción:**
El teatro musical integra actuación, canto y movimiento en una experiencia multisensorial ideal para aprender inglés de manera dinámica.

**Objetivos:**
- Explorar técnicas básicas de canto, movimiento y actuación
- Familiarizarse con repertorio sencillo en inglés
- Fomentar coordinación grupal y expresión artística

**Resultados esperados:**
- Presentación de segmentos cortos de teatro musical en inglés
- Mejora en entonación y pronunciación mediante el canto
- Aumento de la autoconfianza y el trabajo en equipo

---

### 5.5. Speaking Clearly

**Pretítulo del bloque:** Pronunciación y proyección  
**Título del bloque:** Speaking Clearly  

**Descripción:**
Este taller ofrece herramientas prácticas para mejorar la dicción, la entonación y la proyección vocal al hablar inglés en público.

**Objetivos:**
- Identificar obstáculos de pronunciación y claridad
- Desarrollar técnicas de respiración, dicción y proyección
- Fomentar autoconfianza al hablar en público

**Resultados esperados:**
- Mayor claridad y precisión al hablar inglés
- Uso más consciente de la respiración y la voz
- Más seguridad en exposiciones e intervenciones orales

---

### 5.6. Students on Stage

**Pretítulo del bloque:** Experiencia escénica real  
**Título del bloque:** Students on Stage  

**Descripción:**
La experiencia de presentarse frente a un público convierte el inglés en una vivencia real y memorable.

**Objetivos:**
- Vivir la experiencia de actuación en inglés
- Fortalecer confianza en contextos públicos
- Integrar habilidades adquiridas en otros talleres

**Resultados esperados:**
- Presentación confiada de escenas en inglés
- Desarrollo de habilidades de comunicación y escena
- Memorias positivas vinculadas al aprendizaje del idioma

---

### 5.7. Classroom Guest

**Pretítulo del bloque:** Intervención en aula  
**Título del bloque:** Classroom Guest  

**Descripción:**
La presencia de un artista docente introduce una dinámica novedosa en clase y activa una experiencia corta, creativa y motivadora en inglés.

**Objetivos:**
- Brindar una experiencia distinta y memorable
- Exponer a los estudiantes a dinámicas teatrales y lingüísticas innovadoras
- Reforzar el aprendizaje de manera práctica y divertida

**Resultados esperados:**
- Mayor motivación hacia el aprendizaje del inglés
- Más participación y compromiso en la sesión
- Retroalimentación positiva de estudiantes y docentes

---

### 5.8. Reserva de talleres

**Pretítulo del bloque:** Reserva tu experiencia  
**Título del bloque:** Agenda un taller con Playhouse  

**Texto base:**
Para reservar cualquiera de los talleres, completa el siguiente formulario.

**Link de reserva:**
- `https://forms.gle/WW3pxuo1ukHujp44A`

**Información de tarifas disponible en el archivo base:**
- Taller de 3 horas con 1 pedagogo para grupos de menos de 15 participantes: `150.00 USD`
- Taller de 3 horas con 1 pedagogo para más de 15 participantes, en caso excepcional: `220.00 USD`
- Taller de 3 horas con 2 pedagogos para grupos de hasta 30 participantes: `300.00 USD`

---

## 6. Residencia Artística

**Section ID:** `residencia`  
**Pretítulo:** Residencia artística  
**Título:** Una experiencia inmersiva de tres semanas que culmina en una presentación en la escuela  

**Texto introductorio:**
Aprender una lengua extranjera requiere experiencias inmersivas, dinámicas y significativas. La residencia artística de Playhouse ofrece a los estudiantes una vivencia integral donde teatro, música, narración e improvisación se convierten en herramientas reales para aprender inglés.

**Descripción principal:**
Durante tres semanas, los estudiantes participan en talleres combinados de teatro, storytelling, improvisación, teatro musical y speaking clearly, desarrollando habilidades lingüísticas a través de la creación de una presentación final que se realiza en la institución educativa.

---

### 6.1. Objetivo general

**Pretítulo del bloque:** Objetivo pedagógico  
**Título del bloque:** Desarrollar competencia comunicativa en inglés a través de una experiencia escénica  

**Texto:**
La residencia busca fortalecer la comunicación en inglés mediante una experiencia inmersiva en teatro musical y narración que culmina en una presentación escénica dentro de la escuela.

---

### 6.2. Objetivos específicos

**Pretítulo del bloque:** Resultados del proceso  
**Título del bloque:** Habilidades que se trabajan durante la residencia  

**Lista:**
- Fortalecer pronunciación, fluidez, vocabulario y comprensión auditiva
- Explorar voz, cuerpo, improvisación y construcción de personajes en inglés
- Fomentar creatividad y trabajo en equipo a través de escenas y canciones
- Producir una pieza escénica final que fortalezca la confianza de los estudiantes
- Ofrecer a la escuela una experiencia cultural que integre arte e inglés en un solo proceso

---

### 6.3. Resultados esperados

**Pretítulo del bloque:** Impacto de la experiencia  
**Título del bloque:** Lo que la residencia deja en estudiantes e institución  

**Resultados lingüísticos:**
Los estudiantes demuestran mejoras en claridad, pronunciación y fluidez en inglés.

**Resultados artísticos:**
Los participantes viven el proceso teatral y musical completo, desde ensayos hasta presentación final.

**Resultados personales:**
Se fortalece la confianza al hablar en público y al colaborar en grupo.

**Resultados creativos:**
Se construyen personajes, historias y escenas en inglés de forma colectiva.

**Resultados institucionales:**
La escuela recibe una puesta en escena final que visibiliza el aprendizaje y deja una experiencia inspiradora para su comunidad.

**CTA del bloque:**
- Texto: `Solicitar propuesta institucional`

---

## 7. Materiales educativos

**Section ID:** `materiales`  
**Pretítulo:** Materiales educativos  
**Título:** Recursos para preparar a los estudiantes antes y después de cada espectáculo  

**Texto base:**
En esta sección se reunirán los materiales necesarios para que docentes e instituciones preparen a sus estudiantes antes de asistir a un show y continúen el aprendizaje después de la experiencia.

**Contenido del bloque:**
- Activity Kit para antes y después del espectáculo
- Videos de apoyo
- Canciones para trabajar en clase
- Recursos complementarios para docentes

---

### 7.1. Material disponible: The Three Little Pigs

**Pretítulo del bloque:** Recurso descargable  
**Título del bloque:** Activity Kit y materiales de The Three Little Pigs  

**Contenido:**
- Link de descarga del Activity Kit: `pendiente de insertar`
- Links de YouTube de canciones: `pendiente de insertar`

**Texto de apoyo:**
Para más información sobre materiales o uso pedagógico de los recursos, Playhouse puede ser contactado a través del formulario oficial.

**Link de contacto:**
- `https://forms.gle/ePJY7c7sok2NaBWBA`

---

## 8. Equipo

**Section ID:** `equipo`  
**Pretítulo:** Nuestro equipo  
**Título:** Artistas, pedagogos y creativos que convierten el escenario en una experiencia educativa  

**Texto introductorio:**
El equipo de Playhouse reúne dirección artística, producción, música, coreografía, elenco y acompañamiento académico para asegurar una experiencia sólida, expresiva y formativa.

---

### 8.1. Dirección y liderazgo

**Pretítulo del bloque:** Dirección del proyecto  
**Título del bloque:** Liderazgo artístico y producción  

**Miembros:**
- Andrés Vélez — Fundador y Director General
- Luis Vélez — Cofundador y Productor
- Karla Espinosa — Directora Musical y Entrenadora Vocal
- Josselyn Llacxaguanga — Coreógrafa y Entrenadora de Baile

---

### 8.2. Elenco

**Pretítulo del bloque:** Intérpretes en escena  
**Título del bloque:** Elenco Playhouse  

**Miembros:**
- Alexia Montoya
- Siromani Añazco
- Fernando Silva
- Pablo Sánchez
- Luis Vélez
- Andrés Vélez

---

### 8.3. Equipo académico

**Pretítulo del bloque:** Acompañamiento pedagógico  
**Título del bloque:** Equipo académico de Playhouse  

**Miembros:**
- Michelle López
- Gabriela Jiménez
- Jacob Ramírez
- Andrés Vélez — Coordinador

---

## 9. Galería

**Section ID:** `galeria`  
**Pretítulo:** Galería  
**Título:** Presentaciones, ensayos y momentos que muestran la energía de Playhouse  

**Texto base:**
Esta sección debe mostrar la experiencia Playhouse de forma visual, destacando la emoción del escenario, el trabajo del elenco y la participación de estudiantes y comunidades educativas.

**Bloques visuales a cargar en la web:**
- Presentaciones en escena
- Ensayos
- Talleres con estudiantes
- Residencia artística
- Backstage y preparación
- Interacción con público

**Nota para desarrollo:**
Dejar esta sección preparada para una grilla visual o masonry gallery con lightbox.

---

## 10. Contacto / CTA final

**Section ID:** `contacto`  
**Pretítulo:** Contáctanos  
**Título:** Lleva Playhouse a tu institución, aula o comunidad  

**Texto principal:**
Si quieres llevar una función, taller, residencia artística o experiencia educativa de Playhouse a tu escuela o comunidad, podemos ayudarte a diseñar una propuesta adecuada para tu grupo.

**Canales de contacto:**
- Teléfono: `0939576825`
- Correo: `vaplayhouse@gmail.com`
- Formulario principal: `https://forms.gle/ePJY7c7sok2NaBWBA`

**CTA principal:**
- Texto: `Solicitar información`

**CTA secundario:**
- Texto: `Reservar taller`

---

## 11. Footer

**Section ID:** `footer`  
**Pretítulo:** Playhouse  
**Título:** Teatro educativo en inglés  

**Texto corto de cierre:**
Arte, música y pedagogía para crear experiencias inmersivas, creativas y memorables.

**Datos de contacto:**
- Teléfono: `0939576825`
- Correo: `vaplayhouse@gmail.com`

**Redes sociales:**
- Instagram: `https://www.instagram.com/vaplayhouse/`
- Facebook: `https://www.facebook.com/playhousec`
- TikTok: `https://www.tiktok.com/@playhouse.ec`
- YouTube: `https://www.youtube.com/@playhouse_ec`

**Formulario:**
- `https://forms.gle/ePJY7c7sok2NaBWBA`

**Links internos de footer:**
- Inicio
- Sobre Playhouse
- Espectáculos
- Talleres
- Residencia Artística
- Materiales
- Equipo
- Galería
- Contacto

**Copyright sugerido:**
`© Playhouse – Teatro Educativo. Todos los derechos reservados.`

---

## 12. Orden definitivo de secciones en la página

```txt
1. Hero principal
2. Sobre Playhouse
3. Misión y Visión
4. Espectáculos
5. Talleres
6. Residencia Artística
7. Materiales educativos
8. Equipo
9. Galería
10. Contacto / CTA final
11. Footer
```

---

## 13. Estructura rápida para desarrollo

```txt
sections/
  hero
  about
  purpose
  shows
  workshops
  residency
  materials
  team
  gallery
  contact
  footer
```

---

## 14. Nota final para implementación

Este documento puede usarse como:
- base de contenido para la landing
- fuente para constants/data de secciones
- guía para componentes en Next.js
- base para internacionalización posterior en español / inglés

La prioridad inicial debería ser maquetar primero la versión en español con esta estructura, y después duplicar el contenido para la versión en inglés.
